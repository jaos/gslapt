...
void add_pkg_for_install(GtkWidget *gslapt, gpointer user_data)
{
    ...
    if (installed_pkg == NULL) {
        ...
        if (((ver_cmp = slapt_pkg_t_cmp(installed_pkg, pkg)) < 0) || (global_config->re_install == TRUE)) {
            if (ladd_deps_to_trans(trans, all, installed, pkg) == 0) {
                slapt_vector_t *conflicts = slapt_transaction_t_find_conflicts(trans, all, installed, pkg);
...
void build_package_treeviewlist(GtkWidget *treeview)
{
    ...
    slapt_vector_t_foreach (slapt_pkg_t *, pkg, all) {
        ...
        if (installed_pkg != NULL) {
            int cmp = slapt_pkg_t_cmp(pkg, installed_pkg);
...
void build_searched_treeviewlist(GtkWidget *treeview, gchar *pattern)
{
    ...
    if (pattern != NULL && (strcmp(pattern, "") != 0)) {
        ...
        slapt_vector_t_sort(a_matches, slapt_pkg_t_qsort_cmp);
    ...
    while (valid) {
        ...
        if (a_matches)
            ...
            a_pkg = slapt_get_pkg_by_details(all, name, version, location);

        if (i_matches)
            i_pkg = slapt_get_pkg_by_details(i_matches, name, version, location);
        else
            i_pkg = slapt_get_pkg_by_details(installed, name, version, location);

        if (view_list_installed && (i_pkg != NULL || series_match == TRUE)) {
            make_visible = TRUE;
        } else if (view_list_available && (a_pkg != NULL || series_match == TRUE)) {
            make_visible = TRUE;
        } else if (view_list_all && (a_pkg != NULL || i_pkg != NULL || series_match == TRUE)) {
            make_visible = TRUE;
        } else if (view_list_marked && marked && (a_pkg != NULL || i_pkg != NULL || series_match == TRUE)) {
            make_visible = TRUE;
        } else if (view_list_upgradeable && upgradeable && (a_pkg != NULL || series_match == TRUE)) {
            make_visible = TRUE;
        }
        gtk_list_store_set(GTK_LIST_STORE(base_model), &iter, VISIBLE_COLUMN, make_visible, -1);
...
static void fillin_pkg_details(slapt_pkg_t *pkg)
{
    ...
    short_desc = slapt_pkg_t_short_description(pkg);
    if (short_desc != NULL) {
    ...
    if (pkg->installed == false) {
        ...
        gtk_tree_store_insert_with_values(store, &iter, NULL, -1, 0, _("<b>Conflicts:</b>"), -1);
        ...
        gtk_tree_store_insert_with_values(store, &iter, NULL, -1, 0, _("<b>Suggests:</b>"), -1);
    ...
    if ((changelog = slapt_pkg_t_changelog(pkg)) != NULL) {
    ...
    if ((filelist = slapt_pkg_t_filelist(pkg)) != NULL) {
...
static void build_sources_treeviewlist(GtkWidget *treeview)
{
    ...
    slapt_vector_t_foreach (slapt_source_t *, src, global_config->sources) {
        ...
        gtk_list_store_insert_with_values(store, &iter, -1, 0, status_icon, 1, src->url, 2, enabled, 3, priority_str, 4, src->priority, -1);
        // g_object_unref(status_icon);
    }
...
static void build_exclude_treeviewlist(GtkWidget *treeview)
{
    ...
    slapt_vector_t_foreach (char *, exclude, global_config->exclude_list) {
        ...
        gtk_list_store_insert_with_values(store, &iter, -1, 0, exclude, -1);
    }
...
void unmark_package(GtkWidget *gslapt, gpointer user_data)
{
    ...
    if (gtk_tree_selection_get_selected(selection, (GtkTreeModel **)&package_model, &iter) == TRUE) {
        ...
        gtk_list_store_set(GTK_LIST_STORE(model), &actual_iter, STATUS_COLUMN, status, MARKED_COLUMN, FALSE, -1);
        g_free(status);
        ...
        slapt_vector_t_foreach (slapt_pkg_upgrade_t *, upgrade, trans->upgrade_pkgs) {
            if (strcmp(upgrade->installed->name, pkg->name) == 0) {
                ...
                if (set_iter_to_pkg(model, &actual_iter, installed_pkg)) {
                    ...
                    GdkPixbuf *status_icon = gslapt_img("pkg_action_installed.png");
                    gtk_list_store_set(GTK_LIST_STORE(model), &actual_iter, STATUS_ICON_COLUMN, status_icon, STATUS_COLUMN, istatus, MARKED_COLUMN, FALSE, -1);
                    g_free(istatus);
                    g_object_unref(status_icon);
                } else {
...
static void display_dep_error_dialog(slapt_pkg_t *pkg)
{
    ...
    if (result == GTK_RESPONSE_OK) {
        ...
        if (installed_pkg == NULL) {
            ...
            int ver_cmp = slapt_pkg_t_cmp(installed_pkg, pkg);
            slapt_transaction_t_add_upgrade(trans, installed_pkg, pkg);
            if (ver_cmp == 0) {
